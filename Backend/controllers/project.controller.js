...
exports.Add = async (req, res) => {
  try {
    ...
    if (!name || !workspace || !owner) {
      return res
        .status(400)
        .json({ message: "Name, workspace, and owner are required!" });
    }
...
exports.getByWorkspace = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Error fetching projects by workspace",
        error: err.message,
      });
  }
};
...
exports.Archive = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Project archived successfully", archivedProject });
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error archiving project", error: err.message });
  }
};
